    ...
    return new Promise((resolve, reject) => {
        fs.promises.readFile(fileInfo, 'utf-8')
            .then(content => {
                ...
                const filename = fileInfo.split(pathDelimeter)[fileInfo.split(pathDelimeter).length - 1];
...
const saveToFile = (html, outputDir = 'dist', filename) => {
    ...
    fs.promises.writeFile(`${outputDir}${pathDelimeter}${filename}.html`, html)
...
exports.convertFilesToHTML = async (filename, cssUrl, outputDir) => {
    ...
    try {
        if(fs.lstatSync(filename).isDirectory()) {
            const dir = filename.split(pathDelimeter).slice(0, filename.split(pathDelimeter).length).join(pathDelimeter);
